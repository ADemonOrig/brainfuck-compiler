...
struct compiler
{
    string version = "v2.1 windows x64";
    string path = ".\\";
    clock_t start = 0;
    ...
    bool clean() {
        ...
        while (i < size) {
            if (bcode[i] == '+' || bcode[i] == '-') {
                ...
                string opt;
                if (count > 0) opt = string(count, '+');
                else if (count < 0) opt = string(-count, '-');
                code += opt;
            ...
            else if (i + 2 < size && (bcode[i+1] == '+' || bcode[i+1] == '-') && bcode[i+2] == ']') {
                uint s = i;
                while (i + 2 < size && (bcode[i+1] == '+' || bcode[i+1] == '-') && bcode[i+2] == ']') i += 3;
                code += "[-]";
                if (s + 3 != i) change = 1;
            }
    ...
    void compile() {
        ...
        while (i < size) {
            if (code[i] == '+' || code[i] == '-') {
                ...
                else if (count > 1) file << "    *ptr += " << to_string(count) << ";\n";
                else if (count < -1) file << "    *ptr -= " << to_string(-count) << ";\n";
            }
