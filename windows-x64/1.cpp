...
struct compiler
{
    string version = "v2.2 windows x64";
    string path = ".\\";
    clock_t start = 0;
    ...
    bool clean() {
        ...
        while (i < size) {
            if (bcode[i] == '+' || bcode[i] == '-') {
                ...
                string opt;
                if (count > 0) opt = string((unsigned char)count, '+');
                else if (count < 0) opt = string((unsigned char)count, '-');
                code += opt;
    ...
    void compile() {
        ...
        while (i < size) {
            if (code[i] == '+' || code[i] == '-') {
                ...
                if (count == 1) file << "    (*ptr)++;\n";
                else if (count == -1) file << "    (*ptr)--;\n";
                else if (count > 1) file << "    *ptr += " << (unsigned char)count << ";\n";
                else if (count < -1) file << "    *ptr -= " << (unsigned char)count << ";\n";
            }
                ...
                if (count != 0) file << "    ptr = cells + ((ptr - cells + MC) % MC);\n";
            }
                ...
                if (i + 2 < size && (code[i+1] == '+' || code[i+1] == '-') && code[i+2] == ']') {
                    ...
                    if (code[i] == '+' || code[i] == '-') {
                        ...
                        if (count > 0) file << "    *ptr = " << (unsigned char)count << ";\n";
                        else if (count < 0) file << "    *ptr = " << (unsigned char)count << ";\n";
                    }
                    else file << "    *ptr = 0;\n";
                }
                else {
